...
ssh "${SSH_OPTS[@]}" "$REMOTE" \
  "REMOTE_GATEWAY_PATH='$REMOTE_GATEWAY_PATH' COMPOSE_FILE='$COMPOSE_FILE' SERVICE_NAME='$SERVICE_NAME' GATEWAY_GRPC_PORT='$GRPC_PORT' DEPLOY_ENVIRONMENT_NAME='$DEPLOY_ENVIRONMENT_NAME' bash -se" << 'REMOTE_SCRIPT'
set -euo pipefail
