...
El sistema academico esta separado en microservicios. El frontend necesita una entrada unica para autenticar usuarios y consumir servicios como usuarios, catalogo, matriculas, calificaciones y solicitudes.
...
- validacion de JWT contra `academico-usuarios`;
...
Mantener en `academico-usuarios`:
...
- La rotacion de reglas de seguridad se concentra en gateway y usuarios.
- Redis reduce latencia y carga sobre `academico-usuarios`.
...
- Si `academico-usuarios` no esta disponible y el token no esta cacheado, no se pueden validar nuevos accesos protegidos.
- La documentacion y las pruebas deben cubrir el contrato entre gateway y usuarios para evitar regresiones.
...
Posible a futuro si se publica una clave de verificacion estable o JWKS. No se adopta ahora porque la implementacion actual mantiene la autoridad de validacion en `academico-usuarios`.
...
- `academico-usuarios/src/auth/auth.service.js`
